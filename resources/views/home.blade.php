...
<script type="text/javascript">
    ...
    function moveDevices() {
        ...
        $(".devices")[0].onmouseup = function(e) {
            $(".devices .device").popover('enable');
            $(selected_sensor).popover('show');
            selected_sensor = null;
        }
        $(".devices")[0].onmouseleave = function(e) {
            $(".devices .device").popover('enable');
            selected_sensor = null;
        }
        $(".devices .device").each(function() {
    ...
    function changeName(blueprintid, name, cb) {
        ApiRequest("/api/blueprints/" + blueprintid + "/name", "POST", {
            name: name
        }, cb);
    }

    function savePositions(deviceid, x, y, cb) {
        ApiRequest("/api/devices/" + deviceid + "/positions", "POST", {
            x: x,
            y: y
        }, cb);
    }
